...
[MemoryDiagnoser]
[InvocationCount(16)]
public class EnumerateTopKWithPrefixBenchmark
{
    ...
    public List<IReadOnlyList<string>> KTrie_StartsWith()
    {
        ...
        {
            var matches = _kTrie.StartsWith(prefix).Take(K).ToList();
            results.Add(matches);
        }
    ...
    public List<IReadOnlyList<string>> Olve_NaiveArrayTrie_EnumerateWithPrefix()
    {
        ...
        foreach (var prefix in _prefixes)
        {
            var matches = _olveNaiveArrayTrie.EnumerateWithPrefix(prefix).Take(K).ToList();
            results.Add(matches);
        }
